 ...
 * @method     ChildPeriodQuery orderBySequence($order = Criteria::ASC) Order by the sequence column
 ...
 * @method     ChildPeriodQuery groupBySequence() Group by the sequence column
 ...
 *
 * @method     \App\Models\ApplicationQuery|\App\Models\PeriodSchoolYearQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 ...
 * @method     ChildPeriod findOneBySequence(int $sequence) Return the first ChildPeriod filtered by the sequence column
 ...
 * @method     ChildPeriod requireOneBySequence(int $sequence) Return the first ChildPeriod filtered by the sequence column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildPeriod[]|ObjectCollection findBySequence(int $sequence) Return ChildPeriod objects filtered by the sequence column
...
abstract class PeriodQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT id, name, sequence, created_at, updated_at FROM period WHERE id = :p0';
        try {
    ...
    public function filterBySequence($sequence = null, $comparison = null)
    {
        if (is_array($sequence)) {
            ...
            if (isset($sequence['min'])) {
                $this->addUsingAlias(PeriodTableMap::COL_SEQUENCE, $sequence['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($sequence['max'])) {
                $this->addUsingAlias(PeriodTableMap::COL_SEQUENCE, $sequence['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(PeriodTableMap::COL_SEQUENCE, $sequence, $comparison);
    }
