...
class EngagementRequest extends Request {
	...
	public function customValidation($validator){
        $validator->after(function($validator)
        {
			$arr = Request::all();
			$pid = $arr['ProfessorId'];
			$pid_orig = isset($arr['ProfessorIdOrig']) ? $arr['ProfessorIdOrig'] : false;
			$sid = $arr['SubjectId'];
			...
			if($pid !== $pid_orig || $sid !== $sid_orig || $cid !== $cid_orig || $syid !== $syid_orig) {
				...
				if(!is_null($engagement)){
					...
					$validator->errors()->add('SubjectId', 'This combination of professor, subject, course and school year is taken.');
					$validator->errors()->add('CourseId', 'This combination of professor, subject, course and school year is taken.');
					$validator->errors()->add('SchoolYearId', 'This combination of professor, subject, course and school year is taken.');
				}
			}
        });
    }
